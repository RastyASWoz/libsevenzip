...
static void BoolVector_Fill_False(CBoolVector& v, unsigned size) {
    ...
    for (unsigned i = 0; i < size; i++) p[i] = false;
}
...
void CStreamSwitch::Remove() {
    if (_needRemove) {
        if (_archive->_inByteBack->GetRem() != 0) _archive->ThereIsHeaderError = true;
...
void CStreamSwitch::Set(CInArchive* archive, const CObjectVector<CByteBuffer>* dataVector) {
    ...
    if (external != 0) {
        if (!dataVector) ThrowIncorrect();
        const CNum dataIndex = archive->ReadNum();
        if (dataIndex >= dataVector->Size()) ThrowIncorrect();
        Set(archive, (*dataVector)[dataIndex]);
    }
}

void CInArchive::AddByteStream(const Byte* buf, size_t size) {
    if (_numInByteBufs == kNumBufLevelsMax) ThrowIncorrect();
    _inByteBack = &_inByteVector[_numInByteBufs++];
    _inByteBack->Init(buf, size);
}

Byte CInByte2::ReadByte() {
    if (_pos >= _size) ThrowEndOfData();
    return _buffer[_pos++];
}

void CInByte2::ReadBytes(Byte* data, size_t size) {
    if (size == 0) return;
    if (size > _size - _pos) ThrowEndOfData();
...
CNum CInByte2::ReadNum() {
    ...
    */
    ...
    if (value > kNumMax) ThrowUnsupported();
    return (CNum)value;
}

UInt32 CInByte2::ReadUInt32() {
    if (_pos + 4 > _size) ThrowEndOfData();
...
UInt64 CInByte2::ReadUInt64() {
    if (_pos + 8 > _size) ThrowEndOfData();
...
#define IS_SIGNATURE(p) \
    ((p)[2] == Y2 && (p)[3] == Y3 && (p)[5] == Y5 && (p)[4] == Y4 && (p)[1] == Y1 && (p)[0] == Y0)
...
static const Byte* FindSignature_10(const Byte* p, const Byte* limit) {
    for (;;) {
        ...
        if (IS_SIGNATURE(p - 1)) return p - 1;
    }
}
...
static inline bool TestSignature2(const Byte* p) {
    if (!IS_SIGNATURE(p)) return false;
#ifdef FORMAT_7Z_RECOVERY
    if (TestStartCrc(p)) return true;
...
HRESULT CInArchive::FindAndReadSignature(IInStream* stream, const UInt64* searchHeaderSizeLimit) {
    ...
    if (searchHeaderSizeLimit && *searchHeaderSizeLimit == 0) return S_FALSE;
    ...
    for (;;) {
        UInt32 readSize =
            (offset == 0) ? kBufSize - kHeaderSize - kHeaderSize : kBufSize - kHeaderSize;
        if (searchHeaderSizeLimit) {
            const UInt64 rem = *searchHeaderSizeLimit - offset;
            if (readSize > rem) readSize = (UInt32)rem;
            if (readSize == 0) return S_FALSE;
        }

        UInt32 processed = 0;
        RINOK(stream->Read(buf + kHeaderSize, readSize, &processed))
        if (processed == 0) return S_FALSE;
...
void CInByte2::ParseFolder(CFolder& folder) {
    ...
    if (numCoders == 0 || numCoders > k_Scan_NumCoders_MAX) ThrowUnsupported();
    ...
    for (i = 0; i < numCoders; i++) {
        CCoderInfo& coder = folder.Coders[i];
        {
            ...
            if (idSize > 8 || idSize > GetRem()) ThrowUnsupported();
            const Byte* longID = GetPtr();
            UInt64 id = 0;
            for (unsigned j = 0; j < idSize; j++) id = ((id << 8) | longID[j]);
    ...
    if (numInStreams < numBonds) ThrowUnsupported();
    ...
    if (numPackStreams == 1) {
        ...
        if (i == numInStreams) ThrowUnsupported();
    } else
        for (i = 0; i < numPackStreams; i++) folder.PackStreams[i] = ReadNum();
}
...
void CFolders::ParseFolderInfo(unsigned folderIndex, CFolder& folder) const {
    ...
    if (inByte.GetRem() != 0) throw 20120424;
}

void CDatabase::GetPath(unsigned index, UString& path) const {
    path.Empty();
    if (!NameOffsets || !NamesBuf) return;
...
HRESULT CDatabase::GetPath_Prop(unsigned index, PROPVARIANT* path) const throw() {
    ...
    if (!NameOffsets || !NamesBuf) return S_OK;

    const size_t offset = NameOffsets[index];
    const size_t size = NameOffsets[index + 1] - offset;

    if (size >= (1 << 14)) return S_OK;
...
void CInArchive::WaitId(UInt64 id) {
    for (;;) {
        ...
        if (type == id) return;
        if (type == NID::kEnd) ThrowIncorrect();
        SkipData();
    }
}
...
void CInArchive::ReadPackInfo(CFolders& f) {
    ...
    for (CNum i = 0; i < numPackStreams; i++) {
        ...
        sum += packSize;
        if (sum < packSize) ThrowIncorrect();
    }
...
void CInArchive::ReadUnpackInfo(const CObjectVector<CByteBuffer>* dataVector, CFolders& folders) {
    ...
    CNum numCodersOutStreams = 0;
    {
        ...
        for (fo = 0; fo < numFolders; fo++) {
            ...
            for (CNum ci = 0; ci < numCoders; ci++) {
                ...
                const unsigned idSize = (mainByte & 0xF);
                if (idSize > 8) ThrowUnsupported();
                if (idSize > inByte->GetRem()) ThrowEndOfData();
                ...
                if ((mainByte & 0x10) != 0) {
                    ...
                    if (coderInStreams > k_Scan_NumCodersStreams_in_Folder_MAX) ThrowUnsupported();
                    if (inByte->ReadNum() != 1) ThrowUnsupported();
                }

                numInStreams += coderInStreams;
                if (numInStreams > k_Scan_NumCodersStreams_in_Folder_MAX) ThrowUnsupported();

                if ((mainByte & 0x20) != 0) {
                    const CNum propsSize = inByte->ReadNum();
                    if (propsSize > inByte->GetRem()) ThrowEndOfData();
            ...
            if (numCoders == 1 && numInStreams == 1) {
                ...
                for (i = 0; i < numBonds; i++) {
                    CNum index = ReadNum();
                    if (index >= numInStreams || StreamUsed[index]) ThrowUnsupported();
                    StreamUsed[index] = true;

                    index = ReadNum();
                    if (index >= numCoders || CoderUsed[index]) ThrowUnsupported();
                ...
                if (i == numCoders) ThrowUnsupported();
            }

            folders.FoToCoderUnpackSizes[fo] = numCodersOutStreams;
            numCodersOutStreams += numCoders;
            folders.FoStartPackStreamIndex[fo] = packStreamIndex;
            if (numPackStreams > folders.NumPackStreams - packStreamIndex) ThrowIncorrect();
    ...
    for (CNum i = 0; i < numCodersOutStreams; i++) folders.CoderUnpackSizes[i] = ReadNumber();
...
void CInArchive::ReadSubStreamsInfo(CFolders& folders, CRecordVector<UInt64>& unpackSizes,
                                    CUInt32DefVector& digests) {
    folders.NumUnpackStreamsVector.Alloc(folders.NumFolders);
    CNum i;
    for (i = 0; i < folders.NumFolders; i++) folders.NumUnpackStreamsVector[i] = 1;
    ...
    for (;;) {
        ...
        if (type == NID::kNumUnpackStream) {
            for (i = 0; i < folders.NumFolders; i++) folders.NumUnpackStreamsVector[i] = ReadNum();
            continue;
        }
    ...
    if (type == NID::kSize) {
        for (i = 0; i < folders.NumFolders; i++) {
            ...
            for (CNum j = 1; j < numSubstreams; j++) {
                ...
                sum += size;
                if (sum < size) ThrowIncorrect();
            }
            const UInt64 folderUnpackSize = folders.GetFolderUnpackSize(i);
            if (folderUnpackSize < sum) ThrowIncorrect();
        ...
        if (type == NID::kCRC) {
            ...
            for (i = 0; i < folders.NumFolders; i++) {
                ...
                if (numSubstreams == 1 && folders.FolderCRCs.ValidAndDefined(i)) {
                    ...
                    for (CNum j = 0; j < numSubstreams; j++) {
                        bool defined = digests2[k2++];
                        digests.Defs[k] = defined;
                        UInt32 crc = 0;
                        if (defined) crc = ReadUInt32();
                        digests.Vals[k] = crc;
                        k++;
                    }
            }
...
void CInArchive::ReadStreamsInfo(const CObjectVector<CByteBuffer>* dataVector, UInt64& dataOffset,
                                 CFolders& folders, CRecordVector<UInt64>& unpackSizes,
                                 CUInt32DefVector& digests) {
    UInt64 type = ReadID();

    if (type == NID::kPackInfo) {
        dataOffset = ReadNumber();
        if (dataOffset > _rangeLimit) ThrowIncorrect();
...
void CInArchive::ReadBoolVector2(unsigned numItems, CBoolVector& v) {
    ...
    v.ClearAndSetSize(numItems);
    bool* p = &v[0];
    for (unsigned i = 0; i < numItems; i++) p[i] = true;
}
...
void CInArchive::ReadUInt64DefVector(const CObjectVector<CByteBuffer>& dataVector,
    ...
    for (unsigned i = 0; i < numItems; i++) {
        UInt64 t = 0;
        if (defs[i]) t = ReadUInt64();
        p[i] = t;
    }
}

HRESULT CInArchive::ReadAndDecodePackedStreams(
    DECL_EXTERNAL_CODECS_LOC_VARS UInt64 baseOffset, UInt64& dataOffset,
    CObjectVector<CByteBuffer>& dataVector Z7_7Z_DECODER_CRYPRO_VARS_DECL) {
    CFolders folders;
    ...
    for (CNum i = 0; i < folders.NumFolders; i++) {
        ...
        if (unpackSize != unpackSize64) ThrowUnsupported();
        ...
        HRESULT result =
            decoder.Decode(EXTERNAL_CODECS_LOC_VARS _stream, baseOffset + dataOffset, folders, i,
                           NULL,  // &unpackSize64
        ...
        if (dataAfterEnd_Error) ThereIsHeaderError = true;

        if (unpackSize != outStreamSpec->GetPos()) ThrowIncorrect();

        if (folders.FolderCRCs.ValidAndDefined(i))
            if (CrcCalc(data, unpackSize) != folders.FolderCRCs.Vals[i]) ThrowIncorrect();
    }

    if (folders.PackPositions) HeadersSize += folders.PackPositions[folders.NumPackStreams];

    return S_OK;
}

HRESULT CInArchive::ReadHeader(
    DECL_EXTERNAL_CODECS_LOC_VARS CDbEx& db Z7_7Z_DECODER_CRYPRO_VARS_DECL) {
    UInt64 type = ReadID();
    ...
    if (type == NID::kAdditionalStreamsInfo) {
        const HRESULT result = ReadAndDecodePackedStreams(
            EXTERNAL_CODECS_LOC_VARS db.ArcInfo.StartPositionAfterHeader,
            db.ArcInfo.DataStartPosition2, dataVector Z7_7Z_DECODER_CRYPRO_VARS);
        RINOK(result)
    ...
    if (type == NID::kMainStreamsInfo) {
        ReadStreamsInfo(&dataVector, db.ArcInfo.DataStartPosition, (CFolders&)db, unpackSizes,
                        digests);
    ...
    if (type == NID::kFilesInfo) {
        ...
        if (numFiles > 0 && !digests.Defs.IsEmpty()) db.ArcInfo.FileInfoPopIDs.Add(NID::kCRC);
        ...
        for (;;) {
            ...
            const UInt64 size = ReadNumber();
            if (size > _inByteBack->GetRem()) ThrowIncorrect();
                ...
                switch ((UInt32)type2) {
                    case NID::kName: {
                        ...
                        for (i = 0; i < numFiles; i++) {
                            ...
                            const UInt16* buf =
                                (const UInt16*)(const void*)(db.NamesBuf.ConstData() + pos);
                            size_t j;
                            for (j = 0; j < curRem && buf[j] != 0; j++);
                            if (j == curRem) ThrowEndOfData();
                    ...
                    */
                    ...
                    case NID::kAnti:
                        ...
                        break;
                    case NID::kStartPos:
                        ReadUInt64DefVector(dataVector, db.StartPos, (unsigned)numFiles);
                        break;
                    case NID::kCTime:
                        ReadUInt64DefVector(dataVector, db.CTime, (unsigned)numFiles);
                        break;
                    case NID::kATime:
                        ReadUInt64DefVector(dataVector, db.ATime, (unsigned)numFiles);
                        break;
                    case NID::kMTime:
                        ReadUInt64DefVector(dataVector, db.MTime, (unsigned)numFiles);
                        break;
                    case NID::kDummy: {
                        for (UInt64 j = 0; j < size; j++)
                            if (ReadByte() != 0) ThereIsHeaderError = true;
            ...
            if (_inByteBack->GetRem() != 0) ThrowIncorrect();
        }

        type = ReadID();  // Read (NID::kEnd) end of headers

        if (numFiles - numEmptyStreams != unpackSizes.Size()) ThrowUnsupported();
        ...
        if (numAntiItems != 0) db.IsAnti.ClearAndSetSize(numFiles);
...
void CDbEx::FillLinks() {
    ...
    for (i = 0; i < Files.Size(); i++) {
        ...
        if (indexInFolder == 0) {
            ...
            for (;;) {
                if (folderIndex >= NumFolders) ThrowIncorrect();
...
HRESULT CInArchive::ReadDatabase2(
    ...
    if (crcFromArc == 0 && nextHeaderOffset == 0 && nextHeaderSize == 0 && nextHeaderCRC == 0) {
        ...
        if (rem < kCheckSize) checkSize = (unsigned)(rem);
        if (checkSize < 3) return S_FALSE;
        RINOK(InStream_SeekSet(_stream, fileSize - checkSize))
        RINOK(ReadStream_FALSE(_stream, buf, (size_t)checkSize))

        if (buf[checkSize - 1] != 0) return S_FALSE;
    ...
    db.IsArc = false;
    if ((Int64)nextHeaderOffset < 0 || nextHeaderSize > ((UInt64)1 << 62)) return S_FALSE;

    HeadersSize = kHeaderSize;

    if (nextHeaderSize == 0) {
        if (nextHeaderOffset != 0 || nextHeaderCRC != 0) return S_FALSE;
        db.IsArc = true;
    ...
    if (nextHeaderSize_t != nextHeaderSize) return E_OUTOFMEMORY;
    CByteBuffer buffer2(nextHeaderSize_t);

    RINOK(ReadStream_FALSE(_stream, buffer2, nextHeaderSize_t))

    if (CrcCalc(buffer2, nextHeaderSize_t) != nextHeaderCRC) ThrowIncorrect();
    ...
    const UInt64 type = ReadID();
    if (type != NID::kHeader) {
        if (type != NID::kEncodedHeader) ThrowIncorrect();
        const HRESULT result = ReadAndDecodePackedStreams(
            EXTERNAL_CODECS_LOC_VARS db.ArcInfo.StartPositionAfterHeader,
            db.ArcInfo.DataStartPosition2, dataVector Z7_7Z_DECODER_CRYPRO_VARS);
        RINOK(result)
        if (dataVector.Size() == 0) return S_OK;
        if (dataVector.Size() > 1) ThrowIncorrect();
        streamSwitch.Remove();
        streamSwitch.Set(this, dataVector.Front());
        if (ReadID() != NID::kHeader) ThrowIncorrect();
    }
    ...
    return ReadHeader(EXTERNAL_CODECS_LOC_VARS db Z7_7Z_DECODER_CRYPRO_VARS);
}

HRESULT CInArchive::ReadDatabase(
    DECL_EXTERNAL_CODECS_LOC_VARS CDbEx& db Z7_7Z_DECODER_CRYPRO_VARS_DECL) {
    try {
        const HRESULT res = ReadDatabase2(EXTERNAL_CODECS_LOC_VARS db Z7_7Z_DECODER_CRYPRO_VARS);
        if (ThereIsHeaderError) db.ThereIsHeaderError = true;
        if (res == E_NOTIMPL) ThrowUnsupported();
        return res;
