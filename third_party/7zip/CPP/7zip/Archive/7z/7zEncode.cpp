...
void CEncoder::SetFolder(CFolder& folder) {
    ...
    for (i = 0; i < _bindInfo.Coders.Size(); i++) {
        ...
        const NCoderMixer2::CCoderStreamsInfo& coderStreamsInfo =
            _bindInfo.Coders[_bindInfo.Coders.Size() - 1 - i];
...
static HRESULT SetCoderProps2(const CProps& props, const UInt64* dataSizeReduce, IUnknown* coder) {
    Z7_DECL_CMyComPtr_QI_FROM(
        ...
        coder) if (setCoderProperties) return props.SetCoderProps(setCoderProperties,
                                                                  dataSizeReduce);
...
Z7_COM7F_IMF(CMtEncMultiProgress::SetRatioInfo(const UInt64* inSize, const UInt64* /* outSize */)) {
    ...
    if (_progress) return _progress->SetRatioInfo(inSize, &outSize2);
...
HRESULT CEncoder::CreateMixerCoder(DECL_EXTERNAL_CODECS_LOC_VARS const UInt64* inSizeForReduce) {
    ...
    FOR_VECTOR(m, _options.Methods) {
        ...
        if (methodFull.CodecIndex >= 0) {
            RINOK(CreateCoder_Index(EXTERNAL_CODECS_LOC_VARS(unsigned) methodFull.CodecIndex, true,
                                    cod))
        } else {
            RINOK(CreateCoder_Id(EXTERNAL_CODECS_LOC_VARS methodFull.Id, true, cod))
        }
        ...
        if (cod.NumStreams != methodFull.NumStreams) return E_FAIL;

        CMyComPtr<IUnknown> encoderCommon =
            cod.Coder ? (IUnknown*)cod.Coder : (IUnknown*)cod.Coder2;
...
Z7_CLASS_IMP_COM_1(CSequentialOutTempBufferImp2, ISequentialOutStream)
public:
...
Z7_COM7F_IMF(CSequentialOutTempBufferImp2::Write(const void* data, UInt32 size,
                                                 UInt32* processed)) {
    COM_TRY_BEGIN
    if (processed) *processed = 0;
    RINOK(TempBuffer.Write_HRESULT(data, size))
    if (processed) *processed = size;
    if (_mtProgressSpec) _mtProgressSpec->AddOutSize(size);
    return S_OK;
    COM_TRY_END
}

Z7_CLASS_IMP_COM_1(CSequentialOutMtNotify, ISequentialOutStream)
public:
...
Z7_COM7F_IMF(CSequentialOutMtNotify::Write(const void* data, UInt32 size, UInt32* processed)) {
    ...
    if (processed) *processed = realProcessed;
    if (_mtProgressSpec) _mtProgressSpec->AddOutSize(size);
    return res;
}

static HRESULT FillProps_from_Coder(IUnknown* coder, CByteBuffer& props) {
    Z7_DECL_CMyComPtr_QI_FROM(ICompressWriteCoderProperties, writeCoderProperties,
                              coder) if (writeCoderProperties) {
...
HRESULT CEncoder::Encode1(DECL_EXTERNAL_CODECS_LOC_VARS ISequentialInStream* inStream,
                          // const UInt64 *inStreamSize,
                          const UInt64* inSizeForReduce, UInt64 expectedDataSize,
                          CFolder& folderItem,
                          // CRecordVector<UInt64> &coderUnpackSizes,
                          // UInt64 &unpackSize,
                          ISequentialOutStream* outStream, CRecordVector<UInt64>& packSizes,
    ...
    for (i = 0; i < numMethods; i++) _mixer->SetCoderInfo(i, NULL, NULL, false);
    ...
    */
    ...
    for (i = 0; i < numMethods; i++) {
        ...
        */
        {
            Z7_DECL_CMyComPtr_QI_FROM(ICryptoResetInitVector, resetInitVector,
                                      coder) if (resetInitVector) {
                RINOK(resetInitVector->ResetInitVector())
            }
        }
        {
            Z7_DECL_CMyComPtr_QI_FROM(ICompressSetCoderPropertiesOpt, optProps,
                                      coder) if (optProps) {
    ...
    if (_bindInfo.PackStreams.Size() != 0) {
        ...
        outStreamSizeCountSpec->SetStream(
            mtOutStreamNotify.IsDefined() ? mtOutStreamNotify.Interface() : outStream);
        outStreamSizeCountSpec->Init();
        outStreamPointers.Add(outStreamSizeCount);
    }

    for (i = 1; i < _bindInfo.PackStreams.Size(); i++) outStreamPointers.Add(tempBuffers[i - 1]);

    bool dataAfterEnd_Error;

    RINOK(_mixer->Code(&inStreamPointer, outStreamPointers.ConstData(),
                       mtProgress.IsDefined() ? mtProgress.Interface() : compressProgress,
                       dataAfterEnd_Error))

    if (_bindInfo.PackStreams.Size() != 0) packSizes.Add(outStreamSizeCountSpec->GetSize());
...
void CEncoder::Encode_Post(UInt64 unpackSize, CRecordVector<UInt64>& coderUnpackSizes) {
    // unpackSize = 0;
...
HRESULT CEncoder::EncoderConstr() {
    if (_constructed) return S_OK;
    if (_options.Methods.IsEmpty()) {
        // it has only password method;
        if (!_options.PasswordIsDefined) throw 1;
        if (!_options.Bonds.IsEmpty()) throw 1;
        ...
        if (!_options.Bonds.IsEmpty()) {
            ...
            for (i = 0; i < numOutStreams; i++)
                if (_bindInfo.FindBond_for_PackStream(i) == -1) _bindInfo.PackStreams.Add(i);
        }

        if (!_bindInfo.SetUnpackCoder()) return E_INVALIDARG;

        if (!_bindInfo.CalcMapsAndCheck()) return E_INVALIDARG;
        ...
        if (_bindInfo.PackStreams.Size() != 1) {
            ...
            for (;;) {
                if (_bindInfo.Coders[ci].NumStreams == 0) break;
                ...
                if (si >= 0) _bindInfo.PackStreams.MoveToFront((unsigned)si);
                break;
            }
        }
    ...
    if (_bindInfo.Coders.Size() > 16) return E_INVALIDARG;
    if (_bindInfo.GetNum_Bonds_and_PackStreams() > 16) return E_INVALIDARG;
