...
Z7_COM7F_IMF(CHandler::GetPropertyInfo(UInt32 /* index */, BSTR* /* name */, PROPID* /* propID */,
                                       VARTYPE* /* varType */)) {
    return E_NOTIMPL;
}
...
Z7_COM7F_IMF(CHandler::GetArchiveProperty(PROPID propID, PROPVARIANT* value)) {
    ...
    switch (propID) {
        ...
        case kpidPhySize:
            ...
            break;
        case kpidOffset:
            if (_db.ArcInfo.StartPosition != 0) prop = _db.ArcInfo.StartPosition;
            break;
/*
...
static void SetFileTimeProp_From_UInt64Def(PROPVARIANT* prop, const CUInt64DefVector& v,
                                           unsigned index) {
    UInt64 value;
...
bool CHandler::IsFolderEncrypted(CNum folderIndex) const {
    ...
    for (; numCoders != 0; numCoders--) {
        ...
        UInt64 id64 = 0;
        for (unsigned j = 0; j < idSize; j++) id64 = ((id64 << 8) | longID[j]);
...
Z7_COM7F_IMF(CHandler::GetRawProp(UInt32 index, PROPID propID, const void** data, UInt32* dataSize,
                                  UInt32* propType)) {
    *data = NULL;
    *dataSize = 0;
    *propType = 0;
...
HRESULT CHandler::SetMethodToProp(CNum folderIndex, PROPVARIANT* prop) const {
    ...
    if (folderIndex == kNumNoIndex) return S_OK;
    ...
    for (; numCoders != 0; numCoders--, needSpace = true) {
        ...
        if (id64 <= (UInt32)0xFFFFFFFF) {
            ...
            if (id == k_LZMA) {
                ...
                name = "LZMA2";
                if (propsSize == 1) GetLzma2String(s, props[0]);
            } else if (id == k_PPMD) {
                name = "PPMD";
                if (propsSize == 5) {
                    char* dest = s;
                ...
                name = "Delta";
                if (propsSize == 1) ConvertUInt32ToString((UInt32)props[0] + 1, s);
            } else if (id == k_ARM64 || id == k_RISCV) {
                name = id == k_ARM64 ? "ARM64" : "RISCV";
                if (propsSize == 4) ConvertUInt32ToString(GetUi32(props), s);
                /*
                else if (propsSize != 0)
        ...
        if (name) {
            ...
            if (propsLen != 0) totalLen++;
            if (needSpace) totalLen++;
            if (totalLen + 5 >= pos) break;
            pos -= totalLen;
            ...
            if (needSpace) temp[pos + totalLen - 1] = ' ';
        } else {
            AString methodName;
            FindMethod(EXTERNAL_CODECS_VARS id64, methodName);
            if (needSpace) temp[--pos] = ' ';
            if (methodName.IsEmpty())
                ...
                if (len + 5 > pos) break;
                pos -= len;
                for (unsigned i = 0; i < len; i++) temp[pos + i] = methodName[i];
            }
        }
    }
...
Z7_COM7F_IMF(CHandler::GetProperty(UInt32 index, PROPID propID, PROPVARIANT* value)) {
    ...
    */
    ...
    switch (propID) {
        ...
        case kpidPosition: {
            UInt64 v;
            if (_db.StartPos.GetItem(index2, v)) PropVarEm_Set_UInt64(value, v);
            break;
        }
        case kpidCTime:
        ...
        case kpidAttrib:
            ...
            break;
        case kpidCRC:
            if (item.CrcDefined) PropVarEm_Set_UInt32(value, item.Crc);
            break;
        case kpidEncrypted:
            PropVarEm_Set_Bool(value, IsFolderEncrypted(_db.FileIndexToFolderIndexMap[index2]));
            break;
        case kpidIsAnti:
        ...
        case kpidBlock: {
            const CNum folderIndex = _db.FileIndexToFolderIndexMap[index2];
            if (folderIndex != kNumNoIndex) PropVarEm_Set_UInt32(value, (UInt32)folderIndex);
        } break;
            ...
            if (folderIndex != kNumNoIndex) {
                if (_db.FolderStartFileIndex[folderIndex] == (CNum)index2 &&
                    _db.FoStartPackStreamIndex[folderIndex + 1] -
                            _db.FoStartPackStreamIndex[folderIndex] >
                        (propID - kpidPackedSize0)) {
                    PropVarEm_Set_UInt64(
                        value, _db.GetFolderPackStreamSize(folderIndex, propID - kpidPackedSize0));
                }
            } else
                PropVarEm_Set_UInt64(value, 0);
        } break;
...
Z7_COM7F_IMF(CHandler::Open(IInStream* stream, const UInt64* maxCheckStartPosition,
    ...
    try {
        ...
        HRESULT result =
            archive.ReadDatabase(EXTERNAL_CODECS_VARS _db
#ifndef Z7_NO_CRYPTO
                                 ,
