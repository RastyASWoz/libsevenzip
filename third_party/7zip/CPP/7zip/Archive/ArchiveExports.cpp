...
void RegisterArc(const CArcInfo* arcInfo) throw() {
    if (g_NumArcs < kNumArcsMax) {
        ...
        if (p[0] == '7' && p[1] == 'z' && p[2] == 0) g_DefaultArcIndex = g_NumArcs;
        g_Arcs[g_NumArcs++] = arcInfo;
    }
    // else throw 1;
}

Z7_DEFINE_GUID(CLSID_CArchiveHandler, k_7zip_GUID_Data1, k_7zip_GUID_Data2,
...
static inline HRESULT SetPropStrFromBin(const char* s, unsigned size, PROPVARIANT* value) {
    if ((value->bstrVal = ::SysAllocStringByteLen(s, size)) != NULL) value->vt = VT_BSTR;
    return S_OK;
}
...
static int FindFormatCalssId(const GUID* clsid) {
    ...
    if (cls != CLSID_CArchiveHandler) return -1;
    const Byte id = CLS_ARC_ID_ITEM(*clsid);
    for (unsigned i = 0; i < g_NumArcs; i++)
        if (g_Arcs[i]->Id == id) return (int)i;
    return -1;
}
...
STDAPI CreateArchiver(const GUID* clsid, const GUID* iid, void** outObject) {
    COM_TRY_BEGIN {
        ...
        if (!needIn && !needOut) return E_NOINTERFACE;
        const int formatIndex = FindFormatCalssId(clsid);
        if (formatIndex < 0) return CLASS_E_CLASSNOTAVAILABLE;
        ...
        if (needIn) {
            *outObject = arc.CreateInArchive();
            ...
            if (!arc.CreateOutArchive) return CLASS_E_CLASSNOTAVAILABLE;
...
STDAPI GetHandlerProperty2(UInt32 formatIndex, PROPID propID, PROPVARIANT* value) {
    ...
    if (formatIndex >= g_NumArcs) return E_INVALIDARG;
    const CArcInfo& arc = *g_Arcs[formatIndex];
...
// Debug helper: query registered formats
extern "C" unsigned Debug_GetNumRegisteredFormats() {
    return g_NumArcs;
}
