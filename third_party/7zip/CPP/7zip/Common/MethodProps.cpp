...
UInt64 Calc_From_Val_Percents(UInt64 val, UInt64 percents) {
    ...
    if (q != 0) {
        if (val > (UInt64)(Int64)-1 / q) return (UInt64)(Int64)-1;
        res = val * q;
    }
    ...
    if (r != 0) {
        ...
        if (val <= (UInt64)(Int64)-1 / r)
            ...
            v2 = val / 100 * r;
        res += v2;
        if (res < v2) return (UInt64)(Int64)-1;
    }
...
HRESULT PROPVARIANT_to_bool(const PROPVARIANT& prop, bool& dest) {
    switch (prop.vt) {
        case VT_EMPTY:
            ...
            return S_OK;
        case VT_BOOL:
            dest = (prop.boolVal != VARIANT_FALSE);
            return S_OK;
        case VT_BSTR:
            return StringToBool(prop.bstrVal, dest) ? S_OK : E_INVALIDARG;
        default:
            break;
    }
    return E_INVALIDARG;
...
HRESULT ParsePropToUInt32(const UString& name, const PROPVARIANT& prop, UInt32& resValue) {
    ...
    if (prop.vt != VT_EMPTY) return E_INVALIDARG;
    if (name.IsEmpty()) return S_OK;
    UInt32 v;
    if (ParseStringToUInt32(name, v) != name.Len()) return E_INVALIDARG;
    resValue = v;
    return S_OK;
}

HRESULT ParseMtProp2(const UString& name, const PROPVARIANT& prop, UInt32& numThreads,
                     bool& force) {
    force = false;
    UString s;
    if (name.IsEmpty()) {
        ...
        if (prop.vt != VT_BSTR) return res;
        s.SetFromBstr(prop.bstrVal);
        if (s.IsEmpty()) return E_INVALIDARG;
    } else {
        if (prop.vt != VT_EMPTY) return E_INVALIDARG;
        s = name;
    }
...
static HRESULT SetLogSizeProp(UInt64 number, NCOM::CPropVariant& destProp) {
    if (number >= 64) return E_INVALIDARG;
    UInt32 val32;
    if (number < 32) val32 = (UInt32)1 << (unsigned)number;
    /*
...
static HRESULT StringToDictSize(const UString& s, NCOM::CPropVariant& destProp) {
    ...
    if (numDigits == 0 || s.Len() > numDigits + 1) return E_INVALIDARG;

    if (s.Len() == numDigits) return SetLogSizeProp(number, destProp);
    ...
    if (number < range4g) destProp = (UInt32)((UInt32)number << numBits);
    /*
...
static HRESULT PROPVARIANT_to_DictSize(const PROPVARIANT& prop, NCOM::CPropVariant& destProp) {
    if (prop.vt == VT_UI4) return SetLogSizeProp(prop.ulVal, destProp);
...
class CCoderProps {
   ...
   public:
    CCoderProps(unsigned numPropsMax)
        : _propIDs(NULL), _props(NULL), _numProps(0), _numPropsMax(numPropsMax) {
...
void CCoderProps::AddProp(const CProp& prop) {
    if (_numProps >= _numPropsMax) throw 1;
    _propIDs[_numProps] = prop.Id;
    _props[_numProps] = prop.Value;
    _numProps++;
}

HRESULT CProps::SetCoderProps(ICompressSetCoderProperties* scp,
                              const UInt64* dataSizeReduce) const {
    return SetCoderProps_DSReduce_Aff(scp, dataSizeReduce, NULL, NULL, NULL);
}

HRESULT CProps::SetCoderProps_DSReduce_Aff(ICompressSetCoderProperties* scp,
                                           const UInt64* dataSizeReduce, const UInt64* affinity,
    ...
    CCoderProps coderProps(Props.Size() + (dataSizeReduce ? 1 : 0) + (affinity ? 1 : 0) +
                           (affinityGroup ? 1 : 0) + (affinityInGroup ? 1 : 0));
    FOR_VECTOR(i, Props)
...
unsigned CMethodProps::GetLevel() const {
    ...
    if (i < 0) return 5;
    if (Props[(unsigned)i].Value.vt != VT_UI4) return 9;
...
static const CNameToPropID g_NameToPropID[] =
    {
        {VT_UI4, ""},       {VT_UI4, "d"},       {VT_UI4, "mem"},    {VT_UI4, "o"},
        {VT_UI8, "c"},      {VT_UI4, "pb"},      {VT_UI4, "lc"},     {VT_UI4, "lp"},
        {VT_UI4, "fb"},     {VT_BSTR, "mf"},     {VT_UI4, "mc"},     {VT_UI4, "pass"},
        {VT_UI4, "a"},      {VT_UI4, "mt"},      {VT_BOOL, "eos"},   {VT_UI4, "x"},
        {VT_UI8, "reduce"}, {VT_UI8, "expect"},  {VT_UI8, "cc"},  // "cc" in v23,  "b" in v22.01
        {VT_UI4, "check"},  {VT_BSTR, "filter"}, {VT_UI8, "memuse"}, {VT_UI8, "aff"},
        {VT_UI4, "offset"}, {VT_UI4, "zhb"}
        /*
...
/*
#if defined(static_assert) || (defined(__cplusplus) && __cplusplus >= 200410L) || (defined(_MSC_VER)
&& _MSC_VER >= 1600)
...
static int FindPropIdExact(const UString& name) {
    for (unsigned i = 0; i < Z7_ARRAY_SIZE(g_NameToPropID); i++)
        if (StringsAreEqualNoCase_Ascii(name, g_NameToPropID[i].Name)) return (int)i;
    return -1;
}

static bool ConvertProperty(const PROPVARIANT& srcProp, VARTYPE varType,
    ...
    if (varType == VT_BOOL) {
        bool res;
        if (PROPVARIANT_to_bool(srcProp, res) != S_OK) return false;
        destProp = res;
        return true;
    }
...
HRESULT CMethodProps::SetParam(const UString& name, const UString& value) {
    ...
    if (index < 0) {
        ...
        if (!name.IsEqualTo_Ascii_NoCase("b") || value.Find(L':') >= 0) return E_INVALIDARG;
    ...
    if (IsLogSizeProp(prop.Id)) {
        ...
        if (nameToPropID.VarType == VT_BSTR)
            ...
            bool res;
            if (!StringToBool(value, res)) return E_INVALIDARG;
            propValue = res;
        ...
        if (!ConvertProperty(propValue, nameToPropID.VarType, prop.Value)) return E_INVALIDARG;
    }
    Props.Add(prop);
    return S_OK;
}
...
HRESULT CMethodProps::ParseParamsFromPROPVARIANT(const UString& realName,
    ...
    if (IsLogSizeProp(prop.Id)) {
        ...
        if (!ConvertProperty(value, nameToPropID.VarType, prop.Value)) return E_INVALIDARG;
    }
    Props.Add(prop);
    return S_OK;
}
...
UInt64 CMethodProps::Get_Lzma_MemUsage(bool addSlidingWindowSize) const {
    ...
    if (addSlidingWindowSize) {
        ...
        UInt64 blockSize = (UInt64)dict32 + (1 << 16) + (numThreads > 1 ? (1 << 20) : 0);
...
HRESULT COneMethodInfo::ParseMethodFromString(const UString& s) {
    ...
    {
        UString temp = s;
        if (splitPos >= 0) temp.DeleteFrom((unsigned)splitPos);
        if (!temp.IsAscii()) return E_INVALIDARG;
        MethodName.SetFromWStr_if_Ascii(temp);
    }
    if (splitPos < 0) return S_OK;
    PropsString = s.Ptr((unsigned)(splitPos + 1));
    return ParseParamsFromString(PropsString);
}

HRESULT COneMethodInfo::ParseMethodFromPROPVARIANT(const UString& realName,
                                                   const PROPVARIANT& value) {
