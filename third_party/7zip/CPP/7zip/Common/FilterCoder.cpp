...
Z7_COM7F_IMF(CFilterCoder::SetInBufSize(UInt32, UInt32 size)) {
    _inBufSize = size;
    return S_OK;
}
Z7_COM7F_IMF(CFilterCoder::SetOutBufSize(UInt32, UInt32 size)) {
    _outBufSize = size;
    return S_OK;
}
        ...
        */
        ...
        while (readPos != filterPos) {
            ...
            */
            ...
            if (cur > readPos - filterPos) {
                ...
                if (!_encodeMode || cur > _bufSize - filterPos || !inputFinished) {
                    /* (cur > _bufSize - filterPos) is unexpected for AES filter, if _bufSize is
                       multiply of 16. But we support this case, if some future filter will use
                       block with non-power2-size.
                    */
                    needMoreInput = true;
                    break;
                }
                ...
                if (cur != Filter->Filter(buf + filterPos, cur)) return E_FAIL;
            }
            filterPos = f;
        }
        ...
        if (hres == S_OK) {
            ...
            */
            ...
            if (inputFinished) {
                if (!needMoreInput)
                    size = readPos;  // for risc/bcj filters in last block we write data after
                                     // filterPos.
                else if (_encodeMode)
...

        {
            ...
            if (outSize) {
                ...
                if (writeSize > rem) writeSize = (UInt32)rem;
            }
        ...
        // if (size != 0)
        {
            if (filterPos < size) return E_FAIL;  // filterPos = 0; else
            filterPos -= size;
            readPos -= size;
            if (readPos != 0) memmove(_buf, _buf + size, readPos);
        }
        // printf("\nnowPos64=%x, readPos=%x, filterPos=%x\n", (unsigned)nowPos64,
        // (unsigned)readPos, (unsigned)filterPos);
...
HRESULT CFilterCoder::Flush2() {
    while (_convSize != 0) {
        ...
        if (_outSize_Defined) {
            ...
            if (num > rem) num = (UInt32)rem;
            if (num == 0) return k_My_HRESULT_WritingWasCut;
        }

        UInt32 processed = 0;
        const HRESULT res = _outStream->Write(_buf + _convPos, num, &processed);
        if (processed == 0) return res != S_OK ? res : E_FAIL;
    ...
    if (convPos != 0) {
        ...
        Byte* buf = _buf;
        for (UInt32 i = 0; i < num; i++) buf[i] = buf[convPos + i];
        _bufPos = num;
        _convPos = 0;
    }
...
Z7_COM7F_IMF(CFilterCoder::Write(const void* data, UInt32 size, UInt32* processedSize)) {
    ...
    while (size != 0) {
        ...
        if (_bufPos != _bufSize) {
            ...
            if (processedSize) *processedSize += num;
            _bufPos += num;
...
Z7_COM7F_IMF(CFilterCoder::Read(void* data, UInt32 size, UInt32* processedSize)) {
    ...
    while (size != 0) {
        ...
        if (convSize > bufPos) {
            // AES
            if (convSize > _bufSize) return E_FAIL;
            if (!_encodeMode) return S_FALSE;
            Byte* buf = _buf;
            do buf[bufPos] = 0;
...
Z7_COM7F_IMF(CFilterCoder::SetCoderProperties(const PROPID* propIDs, const PROPVARIANT* properties,
