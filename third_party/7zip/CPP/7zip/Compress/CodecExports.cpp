...
// Do NOT define any GUIDs here - they are all defined in codec_guids_manual.cpp
...
static void SetPropFromAscii(const char* s, PROPVARIANT* prop) throw() {
    ...
    if (dest) {
        for (UINT i = 0; i <= len; i++) dest[i] = (Byte)s[i];
        prop->bstrVal = dest;
        prop->vt = VT_BSTR;
    }
}
...
static HRESULT MethodToClassID(UInt16 typeId, CMethodId id, PROPVARIANT* value) throw() {
    ...
    clsId.Data3 = typeId;
    SetUi64(clsId.Data4, id) return SetPropGUID(clsId, value);
}

static HRESULT FindCodecClassId(const GUID* clsid, bool isCoder2, bool isFilter, bool& encode,
                                int& index) throw() {
    index = -1;
    if (clsid->Data1 != k_7zip_GUID_Data1 || clsid->Data2 != k_7zip_GUID_Data2) return S_OK;

    encode = true;

    if (clsid->Data3 == k_7zip_GUID_Data3_Decoder)
        encode = false;
    else if (clsid->Data3 != k_7zip_GUID_Data3_Encoder)
        return S_OK;
    ...
    for (unsigned i = 0; i < g_NumCodecs; i++) {
        ...
        if (codec.NumStreams == 1 ? isCoder2 : !isCoder2) return E_NOINTERFACE;
...
static HRESULT CreateCoder2(bool encode, UInt32 index, const GUID* iid, void** outObject) {
    *outObject = NULL;
    ...
    if (encode ? !codec.CreateEncoder : !codec.CreateDecoder) return CLASS_E_CLASSNOTAVAILABLE;
    ...
    *outObject = NULL;
    ...
    if (!isCoder) {
        ...
        if (!isFilter) {
            ...
            if (!isCoder2) return E_NOINTERFACE;
        }
    }
    ...
    if (codecIndex < 0) return CLASS_E_CLASSNOTAVAILABLE;
...
static int FindHasherClassId(const GUID* clsid) throw() {
    if (clsid->Data1 != k_7zip_GUID_Data1 || clsid->Data2 != k_7zip_GUID_Data2 ||
        clsid->Data3 != k_7zip_GUID_Data3_Hasher)
        return -1;
    const UInt64 id = GetUi64(clsid->Data4);
    for (unsigned i = 0; i < g_NumCodecs; i++)
        if (id == g_Hashers[i]->Id) return (int)i;
    return -1;
}
...
STDAPI CreateHasher(const GUID* clsid, IHasher** outObject) {
    ...
    *outObject = NULL;
    ...
    if (index < 0) return CLASS_E_CLASSNOTAVAILABLE;
...
STDAPI GetModuleProp(PROPID propID, PROPVARIANT* value) {
    ...
    switch (propID) {
        case NModulePropID::kInterfaceType: {
            NWindows::NCOM::PropVarEm_Set_UInt32(
                value, NModuleInterfaceType::k_IUnknown_VirtDestructor_ThisModule);
            break;
        }
